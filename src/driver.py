...
def main():
    ...
    plt.savefig("Raw Earthquake Event Data.png")
    ...
    plt.savefig("Events on Raw Earthquake Event Data.png")
    ...
    # small events can't be detected
    # large events not big sample size
    ...
    n_5 = np.log10(get_counts(M_thresh, magnitudes[int(indices[3]) : ]))
    plt.plot(M_thresh, n_5, "c.-", label="Event 5")

    plt.legend()
    plt.grid()
    plt.savefig("log(N) frequency of events at M magnitude.png")
    ...
    plt.savefig("linear regression subplots1.png")
    ...
    plt.subplot(212)
    plt.plot(M_thresh, n_5, "co", label="Event 5")
    a, e, rsq, model = get_model(n_5, M_thresh)
    plt.plot(M_thresh, model, "k--")
    plt.title(f"Event 5: params: {a}, R^2: {rsq}", loc="left")
    plt.xlabel("magnitude")
    plt.ylabel("log(N) number of events")
    
    plt.savefig("linear regression subplots2.png")
...
def testing_data():
    ...
    Z = np.transpose(
        np.array([[1, 1, 1, 1, 1, 1, 1, 1], [0, 2.3, 4.9, 9.1, 13.7, 18.3, 22.9, 27.2]])
    )
...
def get_counts(threshold, magnitude_slice):
    ...
    for index, M_thresh in enumerate(threshold):
        M_counts = np.append(
            M_counts, np.sum(np.where(magnitude_slice >= threshold[index], 1, 0))
        )
